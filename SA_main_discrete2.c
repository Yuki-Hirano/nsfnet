...
{{0,PROBABILITY},{0,PROBABILITY},{0,PROBABILITY},{0,PROBABILITY},{0,PROBABILITY},{0,PROBABILITY},{0,PROBABILITY},{0,PROBABILITY},{1,PROBABILITY},{0,PROBABILITY},{1,PROBABILITY},{0,PROBABILTY},{PROBABILITY},{0,0.0}},};
...
for(loop=0; loop<LIMIT2; loop++) {
  ...
  src=rand()%N;
  dst=rand()%N;
  while(dst==src || p_link[i][j].cap==0) {
    dst=rand()%N;
  }
...
for(i=0;i<N;i++){
  for( j=0;j<N;j++){
    printf("%d :(%d,%d)---",p_link[i][j].cap,i,j);
    for(a=0;a<N;a++){
      if(PATH[i][j][a]!=INF){
        printf("%d",PATH[i][j][a]);
      }
...
printf("%d,%f,%f,%d,%d\n",model,PROBABILITY,EPSILON,cb_min_temp,end-start);
